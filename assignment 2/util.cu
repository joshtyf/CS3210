 ...
 */
extern "C" int getValueAt(const int *grid, int nRows, int nCols, int row, int col)
{
 ...
 */
extern "C" void setValueAt(int *grid, int nRows, int nCols, int row, int col, int val)
{
    if (row < 0 || row >= nRows || col < 0 || col >= nCols)
    {
        return;
 ...
 */
extern "C" void printWorld(const int *world, int nRows, int nCols)
{
